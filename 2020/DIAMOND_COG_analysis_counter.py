...
for line in infile:
	...
	if line_counter % 1000000 == 0:
		t99 = process_time()
		print(str(line_counter)[:-6] + "M lines processed so far in", t99-t0, "seconds.")
...
for line in db:
	if line.startswith(">") == True:
		...
		db_id = line.strip().replace('>', '').split(" ")[0]
		...
		if "NO COG FOUND" in splitline[1]:
			...
			db_hier = hier_split[1] + " | " + hier_split[2].strip()
		...
		if db_line_counter % 1000000 == 0:							# each million
			t95 = process_time()
			print (str(db_line_counter) + " lines processed so far in", t95-t2, "seconds.")

t3 = process_time()

print ("\nSuccess!")
print ("Time elapsed:", t3-t2, "seconds.")
...
for entry in hit_count_db.keys():
	org = db_hier_dictionary.get(entry)
...
for k, v in sorted(condensed_hit_db.items(), key=lambda kv: -kv[1])[:10]:
	try:
		...
		print ( str(v) + "\tWARNING: Key not found for " + str(k) )
		continue
...
for k, v in sorted(condensed_hit_db.items(), key=lambda kv: -kv[1]):
	try:
		...
		outfile.write (str(q) + "\t" + str(v) + "\t" + str(k) + "\n")
	except KeyError:
		outfile.write (str(q) + "\t" + str(v) + "\tWARNING: Key not found for " + str(k) + "\n")
		error_counter += 1
		continue
